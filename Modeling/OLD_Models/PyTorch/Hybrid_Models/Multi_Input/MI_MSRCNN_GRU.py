...
class MIC_MSRCNN_GRU(nn.Module):
    def __init__(self, 
                 dataset,):
        super(MIC_MSRCNN_GRU, self).__init__()

        self.input_conv = nn.Conv1d(
            in_channels=dataset.price_data_X_train.shape[-1], 
            out_channels=16, 
            kernel_size=1
        )
    ...
    def forward(self, 
                price_data_X):
        input = price_data_X.permute(0, 2, 1) 
