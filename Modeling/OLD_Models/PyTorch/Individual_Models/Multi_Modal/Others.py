...
class MM_TCN(nn.Module):
    def __init__(
        ...
        selected_IMF_X_train = np.expand_dims(
            dataset.price_data_X_train[...,selected_IMF_idx], 
            axis=-1
        )
    ...
    def forward(self, price_data_X):
        current_price_data_X_in = price_data_X[...,self.selected_IMF_idx].unsqueeze(-1)
        current_price_data_X_in = current_price_data_X_in.transpose(1, 2)

        for res_block in self.res_blocks:
            current_price_data_X_in = res_block(current_price_data_X_in)

        out = current_price_data_X_in.transpose(1, 2)
        out = self.fc(out[:, -1, :]) # self.fc(out) / self.fc(out[:, -1, :])
...
#         super(HPT_MM_TCN, self).__init__()
...
#         output_size = selected_IMF_y_train.shape[2]
#         window_size = selected_IMF_X_train.shape[1]
