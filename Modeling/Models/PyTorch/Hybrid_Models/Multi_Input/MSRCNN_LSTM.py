...
class MI_MSRCNN_LSTM(nn.Module):
    def __init__(self, 
                 dataset,):
        super(MI_MSRCNN_LSTM, self).__init__()

        self.input_conv = nn.Conv1d(in_channels=dataset.price_data_X_train.shape[-1], 
                                    out_channels=16, 
                                    kernel_size=1)
        self.relu = nn.ReLU()
        ...
        self.output_fc = nn.Linear(in_features=50, 
                                   out_features=dataset.y_train.shape[2])
...
class HPT_MI_MSRCNN_LSTM(nn.Module):
    def __init__(self, 
                 dataset,
                 ...
                 dense_dims=[],
                 dense_activations=[]):
        super(HPT_MI_MSRCNN_LSTM, self).__init__()
        ...
        if msr_block_cross_channel_fusion:
            input_size = msr_block_conv2d_out_channels
        else:
            input_size = msr_block_conv1d_out_channels * (len([1,2,3]) + 1)

        for l in range(lstm_n_layers):
            self.lstms.append(nn.LSTM(input_size=input_size, 
                                      hidden_size=lstm_dims[l], 
                                      batch_first=True))
            input_size = lstm_dims[l]
        ...
        self.output_fc = nn.Linear(in_features=input_size, 
                                   out_features=dataset.y_train.shape[2])
