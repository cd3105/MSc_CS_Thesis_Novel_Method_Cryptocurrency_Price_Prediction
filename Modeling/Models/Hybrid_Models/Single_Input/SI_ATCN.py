...
class SI_ATCN(nn.Module):
    def __init__(
            self, 
            dataset,
    ):
        super(SI_ATCN, self).__init__()
    ...
    def forward(
        ...
        close_price_data_X.to("cuda")

        close_price_data_X_in = close_price_data_X.transpose(1, 2)

        for res_block in self.res_blocks:
            close_price_data_X_in = res_block(close_price_data_X_in)

        out = close_price_data_X_in.transpose(1, 2)

        out, attention_weights = self.temporal_attention(out)

        #out = self.flatten(out)  
        #out = self.fc(out) 
        out = self.fc(out.mean(dim=1))#(out[:, -1, :]) 
