...
class MIC_TCN_Block(nn.Module):
    def __init__(
        ...
        self.temporal_attention = Single_Head_Temporal_Attention(tcn_n_filters)
    ...
    def forward(
        ...
        current_X_in = X.transpose(1, 2)

        for res_block in self.res_blocks:
            current_X_in = res_block(current_X_in)

        out = current_X_in.transpose(1, 2)
...
class MIC_ATCN(nn.Module):
    def __init__(
            self, 
            dataset,
    ):
        super(MIC_ATCN, self).__init__()

        self.tcn_blocks = nn.ModuleList()
        self.res_blocks = nn.ModuleList()
        self.fcs = nn.ModuleList()

        n_inputs = dataset.close_price_data_X_train.shape[2]
        ...
        self.temporal_attention = Single_Head_Temporal_Attention(n_inputs * 64)
    ...
    def forward(
        ...
        for i, tcn_block in enumerate(self.tcn_blocks):
            current_input = close_price_data_X[..., i].unsqueeze(-1)
        ...
        out = self.fc(out[:,-1,:])#(out.mean(dim=1))#
