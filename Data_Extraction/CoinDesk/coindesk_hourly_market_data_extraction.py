...
for cc in cryptocurrencies_to_retrieve:
    for mk in markets.keys():
        if cc in cryptocurrencies_ts[mk].keys():
            ...
            current_file_path = f"All_Crypto_Data/Crypto_Market_Data/{markets[mk]}/{cc}/1_Hour/"
            current_file_name = f"{markets[mk]}_{cc}{currency_pair_half[mk].replace('-', '_')}_Hourly_UTC_OHLCV_{datetime.utcfromtimestamp(int(list(concat_df['TIMESTAMP'])[0])).strftime('%d-%m-%Y')}__{datetime.utcfromtimestamp(int(list(concat_df['TIMESTAMP'])[-1])).strftime('%d-%m-%Y')}.csv"
