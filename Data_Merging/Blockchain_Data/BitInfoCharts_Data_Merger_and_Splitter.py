...
# Script to Merge BitInfoCharts Data into a single Dataset and split Dataset per cryptocurrency

base_file_path = "All_Crypto_Data/Blockchain_Data/Unmerged/BitInfoCharts/1_Day/"
...
merged_df = merged_df.rename(columns=dict(zip(list(merged_df.columns)[1:],[c.upper() for c in merged_df.columns[1:]])))
...
for cc in ccs:
    ...
    current_cc_path = f"All_Crypto_Data/Blockchain_Data/Merged/BitInfoCharts/{cc}/1_Day/"

    if not os.path.exists(current_cc_path):
        os.makedirs(current_cc_path)
    
    current_cc_df.to_csv(f"{current_cc_path}BitInfoCharts_{cc}_Daily_Blockchain_Data.csv", index=False)
