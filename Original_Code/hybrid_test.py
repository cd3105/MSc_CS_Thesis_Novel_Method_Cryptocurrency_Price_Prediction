        ...
        for c in cryptos: # Loop over Cryptos
            for rep in range(1, 4): # Loop over Number of Repetitions
                ...
                for index, r in enumerate(retrain): # Loop over various values to enlarge Training and shrink Test Set with
                    ...
                    experiment_name = f'hybrid-{c}-{t}-w{str(window)}-h{str(h)}-{str(len(outputs))}m-r{rep}'
                    ...
                    if tuned: # Set Parameters of Model after Tuning
                        parameters = pd.read_csv(f"Original_Code/param/p_HYBRID-{t}-{c}-w{window}-h{h}.csv").iloc[0] 
                        p  = {  'first_lstm_dim': parameters['lstm_dim_1'],
                                'lstm_activation': parameters['lstm_activation'],
                                'first_dropout_rate': parameters['dropout_rate_1'], 
                                'second_lstm_dim': parameters['lstm_dim_2'],
                                'first_dense_dim':  parameters['dense_dim_1'],
                                ...
                                'second_dropout_rate':  parameters['dropout_rate_2'],
                                'second_dense_dim': parameters['dense_dim_2'],
                    ...
                    model = LSTM_GRU(experiment_name) # Initialize LSTM-GRU Object
                ...
                save_results.save_metrics_csv(mses=mse,
                                            ...
                                            filename=experiment_name,
                                            r2=r2_score,
                                            model_type="HYBRID") # Save Metrics
