...
import numpy as np
import pandas as pd
from sklearn.metrics import mean_squared_error, mean_absolute_error, mean_absolute_percentage_error, median_absolute_error, explained_variance_score
from Original_Code.util import save_results
from Original_Code.models.ensembles.voting_regressor import VotingRegressor
...
def get_weighted_ensemble(models, weights, clusters, resources, timeframe =None):
    for res in resources:               
        for c in clusters:
            ...
            vr = VotingRegressor(name = ensemble_name, filename = f_name)
            vr.models = models
            vr.path = 'Original_Code/res/output_'
