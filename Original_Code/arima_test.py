...
def main():
    ...
    cryptos =  ['btc',  'eth', 'ltc', 'xmr', 'xrp'] # Selected Crypto's
    ...
    for t in targets: # Loop of Target Variables
        for c in cryptos: # Loop over Crypto's
            for rep in range(1, 4): # Loop over Number of Repetitions
                ...
                for index, r in enumerate(retrain): # Loop over various values to enlarge Training and shrink Test Set with
                    ...
                    experiment_name = f'arima-{c}-{t}-w{str(window)}-h{str(h)}-{str(len(outputs))}m-r{rep}'
                    ...
                    if tuned: # Set Parameters of Model after Tuning
                        parameters = pd.read_csv(f"Original_Code/param/p_arima-{c}-{t}-w{window}-h{h}-{str(len(outputs))}m.csv").iloc[0] 
                        p = {'p': parameters['p'],
                            'd': parameters['d'],
                            'q': parameters['q'],
                            'P': parameters['P'],
                    ...
                    else: # If Model hasn't been Tuned, then use set these Parameters
                        p = {'p': 1,
                            'd': 0,
                            'q': 2,
                            'P': 2,
                    ...
                    yhat, prediction_std, train_model = model.training(p = p, X_test=to_predict) # Train Model and Retrieve both the Trained Model and the corresponding made Predictions

                    if p['horizon'] > 0: # Restrict Predictions based on Horizon
                        yhat=yhat[:-p['horizon']]
                ...
                save_results.save_metrics_csv(mses=n_mse,
                                            ...
                                            filename=n_experiment_name,
                                            r2=n_r2_score,
                                            model_type="ARIMA") # Save Normalized Metrics
