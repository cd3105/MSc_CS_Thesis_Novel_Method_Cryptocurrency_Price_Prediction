...
class GRU(ModelInterfaceDL):
    ...
    def create_model(self):
        ...
        self.temp_model = Sequential([
            tf.keras.layers.GRU(self.p['first_gru_dim'], activation = self.p['gru_activation'], input_shape = input_shape), # Set up GRU Layer with selected Hyper Parameters
            tf.keras.layers.Dense(self.p['first_dense_dim'], activation=self.p['first_dense_activation']), # Set up Dense Layer after GRU Layer with selected Hyper Parameters
            tf.keras.layers.Dense(self.ds.y_train.shape[2]), # Set up Ouput Dense Layer resulting in an output matching the dimensions of y
        ])
