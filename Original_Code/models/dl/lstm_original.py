...
class LSTM(ModelInterfaceDL):
    ...
    def create_model(self):
        ...
        self.temp_model = Sequential([
            tf.keras.layers.Conv1D(filters=self.p['first_conv_dim'], 
                                   kernel_size=int(self.p['first_conv_kernel']),
                                   strides=1, 
                                   padding="causal",
                                   activation=self.p['first_conv_activation'],
                                   input_shape=input_shape),
            tf.keras.layers.LSTM(int(self.p['first_lstm_dim'])),
            tf.keras.layers.Dense(int(self.p['first_dense_dim']), 
                                  activation=self.p['first_dense_activation']),
            tf.keras.layers.Dense(int(self.ds.y_train.shape[2])),
        ])
