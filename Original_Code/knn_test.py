            ...
            for c in cryptos: # Loop over Cryptos
                for rep in range(1, 4): # Loop over Number of Repetitions
                    ...
                    for index, r in enumerate(retrain): # Loop over various values to enlarge Training and shrink Test Set with
                        ...
                        experiment_name = f'knn-{c}-{t}-w{str(window)}-h{str(h)}-{str(len(outputs))}m-r{rep}'
                        ...
                        if tuned: # Set Parameters of Model after Tuning
                            parameters = pd.read_csv(f"Original_Code/param/p_KNN-{t}-{c}-w{window}-h{h}.csv").iloc[0]
                        ...
                        model = KNN(experiment_name) # Initialize KNN Object
                        model.ds = ds # Set DataSet used for Training to Initialized DataSet Object
                        ds.dataset_creation(df=True, 
                                            detrended= True, 
                                            corrected_setup=s) # Create Training and Test Sets in varying formats with Differenced DataFrame
                        ...
                        ds.dataset_creation(df=True, 
                                            corrected_setup=s) # Create Training and Test Sets in varying formats with Original DataFrame
                    ...
                    if not tuned:
                        save_results.save_params_csv(model.p, model.name) # Save Parameters to File
