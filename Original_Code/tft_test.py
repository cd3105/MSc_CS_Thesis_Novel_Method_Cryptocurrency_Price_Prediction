...
def main():
    ...
    tuned =  0 # Designation of whether the model has been tuned or not with 0 indicating the latter
        ...
        for c in cryptos: # Loop over Cryptos
            for rep in range(1, 4): # Loop over Number of Repetitions
                ...
                for index, r in enumerate(retrain): # Loop over various values to enlarge Training and shrink Test Set with
                    ...
                    experiment_name = f'tft-{c}-{t}-w{str(window)}-h{str(h)}-{str(len(outputs))}m-r{rep}'
                    ...
                    if tuned: # Set Parameters of Model after Tuning
                        parameters = pd.read_csv(f"Original_Code/param/p_TFT-{t}-{c}-w{window}-h{h}.csv").iloc[0] 
                    ...
                    model = TFT(experiment_name) # Initialize TFT Object
                    ...
                    preds = yhat.values().reshape(-1, 1) # Reshape Predictions Array
