            ...
            if f == '1_Day': 
                ...
                for i, (k, alpha, tau, w) in enumerate(combinations):
                    ...
                    for j, imf in enumerate(imfs):
                        current_imf_p_val = adfuller(imf)[1]
                ...
                ).sort_values(by=['Non_Stationary_IMFs', 'MSE', 'IMF_1_P_Val'], ascending=[True, True, True]).reset_index(drop=True)

                current_results_path = f"Exploratory_Data_Analysis/Results/Optimal_VMD/Stationarity_Based/Windowed/{e}/{cc}/"

                if not os.path.exists(current_results_path):
                    os.makedirs(current_results_path)

                if f == '1_Day':
                    current_results_df.to_csv(f"{current_results_path}ADF_Stationarity_{cc}_Daily_Close_Price.csv")
