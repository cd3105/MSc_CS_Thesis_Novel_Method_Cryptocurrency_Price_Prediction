...
correlation_base_path = "Exploratory_Data_Analysis/Results/Correlation_Between_Cryptos/425CCs/Retrieved_Correlation/"
selected_cryptos = ['BTC', 'ETH', 'XRP'] # LTC & XMR Unavailable
    ...
    if corr == 'Granger_Causality':
        for cc in selected_cryptos:
            ...
            for csv in os.listdir(f"{correlation_base_path}{corr}/{cc}/"):
                ...
                current_ranked_corr_fp = f"Exploratory_Data_Analysis/Results/Correlation_Between_Cryptos/425CCs/Ranked_Correlation/{corr}/{cc}/{csv[4:-4]}/"
        ...
        for csv in os.listdir(f"{correlation_base_path}{corr}/"):
            ...
            for cc in selected_cryptos:
                print(f"\t\t- Crypto: {cc}")
                current_corr_df = pd.read_csv(f"{correlation_base_path}{corr}/{csv}", 
                                            index_col=0)
                
                current_ranked_corr_df = current_corr_df[[cc]].sort_values(cc, 
                                                                           ascending=False).iloc[1:]
                current_ranked_corr_fp = f"Exploratory_Data_Analysis/Results/Correlation_Between_Cryptos/425CCs/Ranked_Correlation/{corr}/{cc}/"
