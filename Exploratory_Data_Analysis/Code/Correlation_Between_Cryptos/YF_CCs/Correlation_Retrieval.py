...
for cc in [f for f in os.listdir(base_data_path) if not f.endswith(".txt")]:
    if cc in list(launch_date_df['CC']):
        if (launch_date_df[launch_date_df['CC'] == cc]['LAUNCH_DATE'].iloc[0] <= datetime(2017, 6, 1)):
            file_name = os.listdir(f"{base_data_path}/{cc}/1_Day/")[0]
            cc_df = pd.read_csv(f"{base_data_path}/{cc}/1_Day/{file_name}", 
                                parse_dates=['TIMESTAMP'], 
                                index_col='TIMESTAMP')
    
            cc_df = cc_df.pct_change(fill_method=None).reset_index().iloc[1:]
            cc_df['TIMESTAMP'] = cc_df['TIMESTAMP'].dt.tz_localize(None)
...
price_types = {'Opening_Price': opening_price_df[(opening_price_df['TIMESTAMP'] >= datetime(2017, 6, 1)) & (opening_price_df['TIMESTAMP'] < datetime(2025, 1, 1))].drop('TIMESTAMP',
               ...
               'Closing_Price': closing_price_df[(closing_price_df['TIMESTAMP'] >= datetime(2017, 6, 1)) & (closing_price_df['TIMESTAMP'] < datetime(2025, 1, 1))].drop('TIMESTAMP', 
                                                                                                                                                                        axis=1),}
base_save_path_pc = "Exploratory_Data_Analysis/Results/Correlation_Between_Cryptos/YF_CCs/Retrieved_Correlation/Pearson_Correlation/"
base_save_path_sc = "Exploratory_Data_Analysis/Results/Correlation_Between_Cryptos/YF_CCs/Retrieved_Correlation/Spearman_Correlation/"
base_save_path_gct = "Exploratory_Data_Analysis/Results/Correlation_Between_Cryptos/YF_CCs/Retrieved_Correlation/Granger_Causality/"
all_pcs = []
all_scs = []
all_gcts = {'BTC': [],
            'ETH': [],
