...
base_path = "All_Crypto_Data/Crypto_Market_Data/Merged/Investing/"
...
for cc in [f for f in os.listdir(base_path) if not f.endswith('.txt')]:
    ...
    current_cc_df = pd.read_csv(f"{base_path}{cc}/1_Day/{current_cc_csv}").drop(columns=["Vol.", 
                                                                                         "Change %"])
    ...
    current_cc_df[f'{cc}_ClOSE_PRICE_USD'] = current_cc_df[f'{cc}_CLOSE_PRICE_USD'].apply(lambda x: float(str(x).replace(',', '')))
    current_cc_df[f'{cc}_OPEN_PRICE_USD'] = current_cc_df[f'{cc}_OPEN_PRICE_USD'].apply(lambda x: float(str(x).replace(',', '')))
    current_cc_df[f'{cc}_HIGH_PRICE_USD'] = current_cc_df[f'{cc}_HIGH_PRICE_USD'].apply(lambda x: float(str(x).replace(',', '')))
    current_cc_df[f'{cc}_LOW_PRICE_USD'] = current_cc_df[f'{cc}_LOW_PRICE_USD'].apply(lambda x: float(str(x).replace(',', '')))
